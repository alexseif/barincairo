...
**Feature Goal**: Replace the current subscription input box with a personal, warm, and friendly direct contact CTA connecting visitors to a curated Downtown Cairo Bar Crawl via WhatsApp (`wa.me`) and Email (`mailto:`).  
...
Provide an engaging, conversion-focused interim contact section on `barincairo.com`. Instead of a generic input box, this section welcomes visitors with a personal, friendly tone, inviting them to connect directly on WhatsApp for a curated, fun Downtown Cairo bar crawl and nightlife heritage walk.
...
- **Body Copy**: *"Hey there! We love Downtown Cairo's historic bars, hidden passage bistros, and rooftop breezes. Looking for a friendly, curated night out? Message us directly on WhatsApp and let's plan a fun bar crawl tailored to your vibe!"*
- **Primary CTA Button**: *"Chat on WhatsApp"* (`https://wa.me/...` with pre-filled message: `"Hey! I'm on barincairo.com and would love to organize a curated bar crawl!"`)
- **Secondary CTA Link**: *"Or send an email"* (`mailto:${process.env.NEXT_PUBLIC_CONTACT_EMAIL}?subject=Curated%20Cairo%20Bar%20Hop`)
...
export const CONTACT_CONFIG = {
  ...
  CONTACT_EMAIL: process.env.NEXT_PUBLIC_CONTACT_EMAIL || 'hello@barincairo.com',
  DEFAULT_WA_MESSAGE: 'Hey! I am on barincairo.com and would love to organize a curated bar crawl!',
}
```
...
- [ ] **SEC-1.4**: Phone numbers and email defaults loaded strictly via environment configuration (`NEXT_PUBLIC_WHATSAPP_NUMBER`, `NEXT_PUBLIC_CONTACT_EMAIL`) without hardcoded personal contact details.
