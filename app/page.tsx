    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <header className="relative z-20 border-b border-border/70 bg-background/95">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-5 lg:px-10">
          ...
          <nav className={`${mobileNav ? 'flex' : 'hidden'} absolute left-0 right-0 top-full flex-col gap-5 border-b border-border bg-background px-5 py-5 font-mono text-[10px] uppercase tracking-[0.18em] md:static md:flex md:flex-row md:items-center md:gap-8 md:border-0 md:bg-transparent md:p-0`}>
            ...
            <a href="#subscribe" className="border border-primary px-4 py-2 text-primary transition-colors hover:bg-primary hover:text-primary-foreground">
              WhatsApp Dispatch <ArrowUpRight className="ml-1 inline size-3" />
            </a>
          </nav>

          <button className="md:hidden p-2 min-h-[44px] min-w-[44px] flex items-center justify-center" onClick={() => setMobileNav(!mobileNav)} aria-label={mobileNav ? 'Close menu' : 'Open menu'}>
            {mobileNav ? <X /> : <Menu />}
          </button>
        </div>
      </header>
      ...
      <section id="top" className="mx-auto grid max-w-[1440px] gap-10 px-5 pb-14 pt-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-end lg:px-10 lg:pb-20 lg:pt-24">
        ...
        <div className="flex max-w-lg flex-col gap-6 lg:pb-2 lg:pl-14">
          <p className="font-serif text-xl leading-relaxed text-primary/80 lg:text-2xl">
            A living spatial index to the bars, backrooms, rooftop corners, and historic spots of Wust El Balad, Cairo.
          </p>
          <div className="flex items-center gap-3 border-t border-border pt-4 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
            <span className="size-2 rounded-full bg-accent" /> First Edition · PostGIS Vector Stream
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-5 lg:px-10">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-primary/20 pb-4">
            <div className="flex items-center gap-3">
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">Plate 01</span>
            ...
            <div className="flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-[0.16em]">
              <span className="text-muted-foreground flex items-center gap-1 mr-1"><DollarSign className="size-3" /> Price:</span>
              {PRICE_FILTERS.map((p) => (
                <button
                  key={p.slug}
          ...
          <div className="my-4 flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-[0.14em]">
            <span className="text-muted-foreground flex items-center gap-1 mr-1"><Tag className="size-3 text-accent" /> Vibes:</span>
            {VIBE_FILTERS.map((v) => (
              <button
                key={v.slug}
      ...
      <section className="mx-auto grid max-w-[1440px] gap-10 px-5 py-16 lg:grid-cols-[0.7fr_1.3fr] lg:px-10 lg:py-24">
        <div className="flex flex-col justify-between gap-8">
          <div>
            <p className="mb-4 font-mono text-[10px] uppercase tracking-[0.2em] text-accent">Selected from the map</p>
        ...
        {selectedProps && (
          <article className="grid overflow-hidden border border-primary/25 bg-card sm:grid-cols-[0.85fr_1.15fr]">
            <div
              ...
              style={{ backgroundImage: `url(${selectedProps.photo_url || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?auto=format&fit=crop&w=900&q=80'})` }}
              role="img"
            ...
            <div className="flex flex-col justify-between gap-8 p-6 lg:p-9">
              <div>
                <div className="mb-5 flex items-start justify-between gap-3">
                  ...
                  <button className="border border-primary/30 p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-primary transition-colors hover:bg-primary hover:text-primary-foreground" aria-label="Search venue details">
              ...
              <div>
                <div className="mb-5 flex flex-wrap gap-2">
                  ...
                  {selectedProps.vibes.map((vibeSlug) => (
                    <span key={vibeSlug} className="border border-accent/40 bg-accent/10 px-2 py-1 font-mono text-[9px] uppercase tracking-[0.13em] text-accent">
                      #{vibeSlug}
                    </span>
                  ))}
      ...
      <section id="bar-hops" className="border-y border-primary/25 bg-primary px-5 py-16 text-primary-foreground lg:px-10 lg:py-24">
        <div className="mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[1fr_1fr] lg:items-center">
          ...
          <div className="max-w-lg lg:justify-self-end">
            <p className="mb-6 font-serif text-xl leading-relaxed text-primary-foreground/80">
              Join a small group of curious people as we follow a handpicked trail through Downtown’s after-hours institutions.
            </p>
            <div className="flex flex-wrap gap-4 font-mono text-[10px] uppercase tracking-[0.16em] text-primary-foreground/70">
              <span className="flex items-center gap-2"><Clock3 className="size-4 text-accent" /> 4 hours</span>
              <span className="flex items-center gap-2"><MapPin className="size-4 text-accent" /> 4 stops</span>
              <a href="#subscribe" className="flex items-center gap-2 text-accent hover:text-primary-foreground">
      ...
      <footer id="about" className="border-t border-border px-5 py-8 lg:px-10">
        <div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-5 sm:flex-row sm:items-center">
          ...
          <a href="#top" className="font-mono text-[9px] uppercase tracking-[0.16em] text-muted-foreground hover:text-primary">
